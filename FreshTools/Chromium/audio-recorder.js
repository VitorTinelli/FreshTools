(() => {
  ...
  function recorderConfiguration() {
    const nativeType = "audio/ogg;codecs=opus";
    if (window.MediaRecorder?.isTypeSupported(nativeType)) {
      return { Recorder: window.MediaRecorder, mimeType: nativeType, workerOptions: undefined };
    }

    const Recorder = globalThis.OpusMediaRecorder || self.OpusMediaRecorder || window.OpusMediaRecorder;
    const runtime = globalThis.chrome?.runtime || globalThis.browser?.runtime;
    const encoderWorker = Recorder?.encoderWorker || globalThis.encoderWorker || self.encoderWorker;
    if (typeof Recorder !== "function" || typeof encoderWorker !== "function" || !runtime?.getURL) {
      throw new Error("Codificador OGG não foi carregado.");
    }
    // Chrome may expose the two UMD bundles as separate global properties.
    Recorder.encoderWorker = encoderWorker;
    return {
      Recorder,
      mimeType: "audio/ogg",
      workerOptions: {
        OggOpusEncoderWasmPath: runtime.getURL("vendor/OggOpusEncoder.wasm")
      }
    };
  }
  ...
  async function start(button) {
    ...
    try {
      ...
      const type = config.mimeType;
      current.recorder = new config.Recorder(current.stream,
        { mimeType: type, audioBitsPerSecond: 64000 }, config.workerOptions);
